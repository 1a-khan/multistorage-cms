...
class CloudProviderTests(TestCase):
    ...
    def test_s3_provider_upload_calls_boto3_client_with_expected_arguments(self):
        backend = StorageBackend.objects.create(
            name='S3 Backend',
    ...
    def test_google_drive_provider_upload_calls_google_api_and_returns_drive_uri(self):
        backend = StorageBackend.objects.create(
            name='Drive Backend',
        ...
        source = Path('/tmp/multistorage-cms-gdrive-source.txt')
        ...
        try:
            ...
            fake_drive_service.files.assert_called_once()
            fake_drive_files.create.assert_called_once()
        finally:
            source.unlink(missing_ok=True)
